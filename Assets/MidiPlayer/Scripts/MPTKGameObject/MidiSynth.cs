...
namespace MidiPlayerTK
{
    ...
    public partial class MidiSynth : MonoBehaviour
    {
        ...
        /// <summary>
        /// [obsolete] replaced with MPTK_Spatialize"); 
        /// </summary>
        [HideInInspector]
        public bool MPTK_PauseOnDistance
        {
            get { Debug.LogWarning("MPTK_PauseOnDistance is obsolete, replaced with MPTK_Spatialize"); return spatialize; }
            set { Debug.LogWarning("MPTK_PauseOnDistance is obsolete, replaced with MPTK_Spatialize"); spatialize = value; }
        }
        /// <summary>
        ...
        public fluid_synth_status state;                /** the synthesizer state */
        ...
        public void Start()
        {
            try
            {
                ...
                //if (!noMidi)
                //{
                //    synth1 = Instantiate<MidiSynth>(this);
                //    synth1.noMidi = true;
                //    synth1.MPTK_DirectSendToPlayer = false;
                //}
        ...
        public void MPTK_InitSynth(int channelCount = 16)
        {
            ...
            if (VerboseSynth) Debug.LogFormat("MPTK_InitSynth. Init: {0}, Channels: {1}", fluid_synth_initialized, Channels.Length);
        ...
        {
            if (VerboseSynth) Debug.LogFormat("MPTK_InitSequencerMidi {0} {1}", fluid_synth_initialized, "thread is " + (midiThread == null ? "null" : "alive:" + midiThread.IsAlive));

            if (midiThread == null || !midiThread.IsAlive)
            {
                if (VerboseSynth) Debug.LogFormat("MPTK_InitSequencerMidi {0}", fluid_synth_initialized);
        ...
        {
            if (Channels != null && CheckParamChannel(0))
        ...
        {
            //Debug.LogFormat("ThreadMidiPlayer {0} start midi thread", fluid_synth_initialized);
            ...
            while (state == fluid_synth_status.FLUID_SYNTH_PLAYING)
            {
                ...
                //if (miditoplay.ReadyToPlay) Debug.Log("ThreadMidiPlayer watchMidi:" + Math.Round(nowMs, 2) + " lastTimeMidi:" + Math.Round(lastTimeMidi, 2) + " timeMidiFromStartPlay:" + Math.Round(timeMidiFromStartPlay, 2) + " delta:" + Math.Round(StatDeltaThreadMidiMS, 2));
                lastTimeMidi = nowMs;
            ...
            //Debug.LogFormat("ThreadMidiPlayer {0} stop midi thread", fluid_synth_initialized);
